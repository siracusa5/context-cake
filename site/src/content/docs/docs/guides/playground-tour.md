...
removing a source edits `playground/manifest.json` in place.
...
- **GitHub repo** — cloned with `git` (uses your credentials, so private repos
  ...
  in `playground/.cache/` (gitignored). Only the `owner/name`, https, and SSH
...
vendored under `playground/vendor/tiktoken/`.
...
- **Sync** re-reads the sources from disk, so you can edit the demo OKF
  markdown in `playground/demo-layers/` and watch the cascade change on
  refresh.
...
`playground/vendor/`: no CDN, no npm, works offline. The engine itself stays
dependency-free.
